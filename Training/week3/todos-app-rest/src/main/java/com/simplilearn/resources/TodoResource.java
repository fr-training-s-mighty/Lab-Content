...
public class TodoResource {
	...
	@GetMapping("/{id}")
	public ResponseEntity<Boolean> getTodoById(@PathVariable Integer id) {
		boolean todo = todoService.findById(id);
	...
	public ResponseEntity<Void> createTodo(@RequestBody Todo theTodo) {
		...
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").build(theTodo.getId());
